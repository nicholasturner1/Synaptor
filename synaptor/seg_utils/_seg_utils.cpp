...
Compute 2D manhattan distances to the nearest segment within the second
...
const char* manhattan_dist3d__doc__ = R"/(
Compute manhattan distance to nearest segment in-place.

Take a segmentation volume along with another volume of the same size.
Compute 3D manhattan distances to the nearest segment within the second
volume, and record the closest segment for each index within the
segmentation volume.
...
template<typename T, typename S>
py::array_t<T> manhattan_dist3d(py::array_t<S> seg, py::array_t<T> dists)
{
    auto rs = seg.template mutable_unchecked<3>();
    auto rd = dists.template mutable_unchecked<3>();
    ...
    for (ssize_t i = 0; i < rs.shape(0); ++i){
        for (ssize_t j = 0; j < rs.shape(1); ++j){
            for (ssize_t k = 0; k < rs.shape(2); ++k){
                ...
                if (i > 0 && rd(i-1, j, k) < rd(i, j, k)){
                    rd(i, j, k) = rd(i-1, j, k)+1;
                    rs(i, j, k) = rs(i-1, j, k);
                }
            }
        }
...
const char* dilate_by_k3d__doc__ = R"/(
Dilate a segmentation by manhattan distance in 3D in-place.
...
template<typename T, typename S>
py::array_t<T> dilate_by_k3d(py::array_t<T> seg, py::array_t<T> dists,
                              unsigned int dil_k)
{
    dists = manhattan_dist3d(seg, dists);
...
PYBIND11_MODULE(_seg_utils, m) {
    ...
    m.def("manhattan_distance3d",
          &manhattan_dist3d<unsigned int, unsigned int>);
    m.def("manhattan_distance3d",
          &manhattan_dist3d<unsigned int, float>,
          manhattan_dist3d__doc__);
    ...
    m.def("dilate_by_k", &dilate_by_k<float, float>,
          dilate_by_k__doc__);
    m.def("dilate_by_k3d", &dilate_by_k3d<unsigned int, unsigned int>);
    m.def("dilate_by_k3d", &dilate_by_k3d<float, unsigned int>);
    m.def("dilate_by_k3d", &dilate_by_k3d<unsigned int, float>);
    m.def("dilate_by_k3d", &dilate_by_k3d<float, float>,
          dilate_by_k3d__doc__);
}
